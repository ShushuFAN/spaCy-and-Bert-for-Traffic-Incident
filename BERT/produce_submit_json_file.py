...
def get_latest_model_predict_data_dir(new_epochs_ckpt_dir=None):
    ...
    if new_epochs_ckpt_dir is None:
        ...
        input_new_epochs = os.path.join(
                ...
                os.path.abspath(os.path.join(os.path.dirname(__file__),  "./tmp/bert_project")), "sequnce_infer_out")
        # 获取最新周期文件路径
...
class Sorted_relation_and_entity_list_Management(File_Management):
    ...
    def __init__(self, TEST_DATA_DIR, MODEL_OUTPUT_DIR, Competition_Mode=False):
        ...
        # 关系列表 把模型输出的实数值对应为标签
        self.relationship_label_list = ['Affect','Status','Lane','Direction','Near','Between','Arrangement','Location']
        self.Competition_Mode = Competition_Mode
    ...
    def get_input_list(self,):
        ...
        print("text_sentence_list:%s,token_in_not_NUK_list:%s,token_label_list:%s,reference_spo_list:%s"%
              (len(text_sentence_list),len(token_in_not_NUK_list),len(token_label_list),len(reference_spo_list)))
    ...
    def produce_relationship_and_entity_sort_list(self):
        ...
        for [text_sentence, token_in_not_UNK, token_label, refer_spo_str] in\
            ...
            # print("text_sentence:\n%s\ntoken_in_not_UNK:\n%s"%(text_sentence,token_in_not_UNK))
...
if __name__=='__main__':
    ...
    OUT_RESULTS_DIR = "./tmp/bert_project/final_text_spo_list_result"
